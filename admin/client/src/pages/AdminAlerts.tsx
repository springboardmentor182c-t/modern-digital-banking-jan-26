...
export function AdminAlerts() {
  ...
  const alertTypeCounts = {
    low_balance: systemAlerts.filter((a: any) => a.type === 'low_balance').length,
    bill_due: systemAlerts.filter((a: any) => a.type === 'bill_due').length,
    budget_exceeded: systemAlerts.filter((a: any) => a.type === 'budget_exceeded').length,
  };
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
                          <span className="text-xs font-medium text-primary">
                            {alert.userName.split(' ').map((n: any) => n[0]).join('')}
                          </span>
                        </div>
                        <div>
