        ...
        <Card className="p-6">
          <div className="flex items-start justify-between">
            <div className="space-y-2 flex-1">
              <p className="text-sm text-muted-foreground">Net Savings ({cashFlowData.length || 0} months)</p>
            ...
            <div className="space-y-2 flex-1">
              ...
              <p className="text-3xl font-semibold">
                ₹{cashFlowData.length > 0
                  ? (totalIncome / cashFlowData.length).toLocaleString(undefined, { maximumFractionDigits: 0 })
                  : '0'}
              </p>
              <div className="flex items-center gap-1 text-sm text-blue-600">
                <TrendingUp className="w-4 h-4" />
                <span>Based on transaction data</span>
              </div>
            </div>
        ...
        <Card className="p-6">
          <div className="flex items-start justify-between">
            <div className="space-y-2 flex-1">
              ...
              <p className="text-3xl font-semibold">
                ₹{cashFlowData.length > 0
                  ? (totalExpenses / cashFlowData.length).toLocaleString(undefined, { maximumFractionDigits: 0 })
                  : '0'}
              </p>
              <div className="flex items-center gap-1 text-sm text-green-600">
          ...
          {alerts.length > 0 ? (
            alerts.map((alert) => {
              ...
              return (
                <div
                  key={alert.id}
                  className={`flex gap-4 p-4 rounded-lg border ${alertColors[alert.type as keyof typeof alertColors] || 'bg-blue-50 border-blue-200'}`}
                >
                <Icon className={`w-5 h-5 ${iconColors[alert.type as keyof typeof iconColors] || 'text-blue-600'} flex-shrink-0 mt-0.5`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          {cashFlowData.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-12">No transaction data available for chart</p>
          )}
        </Card>
        ...
        <Card className="p-6">
          ...
          {spendingByCategory.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-12">No spending data available for chart</p>
          )}
        </Card>
      </div>
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge className={`border-0 ${savingsRate >= 20 ? 'bg-green-100 text-green-700' : savingsRate >= 10 ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>
      ...
      <Card className="p-6 bg-gradient-to-br from-blue-50 to-purple-50">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 p-4 bg-white rounded-lg">
            ...
            <div>
              <p className="font-medium mb-1">Track your spending</p>
              <p className="text-sm text-muted-foreground">
                Review your spending categories regularly to identify areas where you can save more.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3 p-4 bg-white rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Based on your savings, consider investing in a diversified portfolio for long-term growth.
              </p>
            </div>
          </div>
        </div>
